...
def get_obs_fn(model: GrandNet, observable_key=None) -> ObservableFn:
    """
...
def get_obs_and_force_fn(model: GrandNet) -> ObservableFn:
    """
    Get an observable function from a `GrandNet` that returns all direct observables and the force. For the force
    function to exist, the `GrandNet` must take the atomic coordinates as input and return the energy as observable.
    The function uses the `get_obs_and_grad_obs_fn` function internally. Note, that the `GrandNet` must have the
    ...
    `reset_prop_keys(new_prop_keys, sub_modules=True)` function of the `GrandNet` module.

    Args:
        model (GrandNet): An initialized `GrandNet`.
