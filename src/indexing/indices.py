...
from src.geometric import coordinates_to_distance_matrix
from src.padding.padding import index_padding_length, pad_indices
...
def get_indices(R: np.ndarray, z: np.ndarray, r_cut: float):
    """
        ...
        coordinates `R` and the atomic types `z` can also be already padded (see `grand.src.padding.padding`) and are
...
def get_indices_big_memory(R: np.ndarray, z: np.ndarray, r_cut: float, batch_size: int = 10000):
    """
        For the `n_data` data frames, return index lists for centering and neighboring atoms given some cutoff radius
        `r_cut` for each structure. As atoms may leave or enter the neighborhood for a given atom within the dataset,
        one can have different lengths for the index lists, even for same structures. Thus, the index lists are padded
    ...
    # divide the data samples into batches
    n_data = R.shape[0]
    n_batches = int(np.ceil(n_data / batch_size))
    R_batches = np.array_split(R, n_batches, axis=0)
    z_batches = np.array_split(z, n_batches, axis=0)
    # compute indices for each batch
    idx_batches = [get_indices(R_batches[i], z_batches[i], r_cut) for i in range(n_batches)]
    # pad the batches to the same length
    pad_length = max([idx_batches[i]['idx_i'].shape[1] for i in range(n_batches)])
    idx_batches = [pad_indices(idx_batches[i]['idx_i'], idx_batches[i]['idx_j'], pad_length) for i in range(n_batches)]
    # concatenate the batches
    pad_idx_i = np.concatenate([idx_batches[i][0] for i in range(n_batches)], axis=0)
    pad_idx_j = np.concatenate([idx_batches[i][1] for i in range(n_batches)], axis=0)

    return {'idx_i': pad_idx_i, 'idx_j': pad_idx_j}, pad_length
