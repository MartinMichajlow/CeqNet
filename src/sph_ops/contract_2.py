...
def init_expansion_fn(degrees: Sequence[int], cg: jnp.ndarray) -> Callable[[jnp.ndarray], jnp.ndarray]:
    ...
    Returns: Expansion function, which takes the outer product of spherical basis functions to the correct expansion
        for arbitrary order l3. Illustrated in more detail in the example.py below.
...
def make_l0_contraction_fn(degrees: Sequence[int]):
    if 0 in degrees:
        # raise ValueError('SPHCs are defined with l > 0.')
        cg = init_clebsch_gordan_matrix(degrees=[*degrees], l_out_max=0)  # shape: (1,m_tot,m_tot)
        ...
        cg = init_clebsch_gordan_matrix(degrees=[0, *degrees], l_out_max=0)  # shape: (1,m_tot,m_tot)
