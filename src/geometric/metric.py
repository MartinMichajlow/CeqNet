...
from src.cutoff_function.pbc import pbc_diff
...
# @jax.jit
def coordinates_to_distance_matrix_masked(x: jnp.ndarray, n_a: int) -> jnp.ndarray:
    """
    Calculate the pairwise distance matrix from the coordinates in d dimensions for n points.

    Args:
        x (array_like): coordinates, shape: (...,n,d)
        n_a (int): number of atoms

    Returns: distance vector matrix between coordinates, shape:  (...,n,n,1)

    """

    D = jnp.sum(coordinates_to_distance_vectors(x[:n_a])**2, axis=-1, keepdims=True)
    return safe_mask(D > 0., jnp.sqrt, D)
